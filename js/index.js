...
document.addEventListener('DOMContentLoaded', async ()=> {
    ...
    async function getGroups(){
        try {
            const response = await axios.get('http://localhost:3000/user/getGroup', {headers: {"Authorization": token}});
    ...
    form.addEventListener('submit', async(e) => {
        ...
        try {
            const response = await axios.post(`http://localhost:3000/user/addGroup`, {group: groupName}, {headers: {"Authorization": token}});
    ...
    groups.addEventListener('click', (e)=> {
        if(e.target.classList.contains('group')){
            ...
            window.location.href= `file:///C:/Users/shubh/OneDrive/Desktop/back-end/chatApp/frontend/chats.html?groupID=${id}`;
        }
    });
});
